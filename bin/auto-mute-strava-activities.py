...
OFFICE_POINT = (COMMUTE_IF_CLOSE_TO != None) and tuple(
    [float(s.strip()) for s in COMMUTE_IF_CLOSE_TO.split(",")]
)
...
def is_close_to_office(activity):
    ...
    return (kms_to_start_point <= MAXIMUM_DISTANCE_IN_KM) or (
        kms_to_end_point <= MAXIMUM_DISTANCE_IN_KM
    )
...
def should_be_marked_as_commute(activity):
    if COMMUTE_IF_CLOSE_TO == None:
        print(
            f"COMMUTE_IF_CLOSE_TO not provided, cannot evaluate activity {activity['id']}"
        )
        return False
    ...
    return (sport_type == "Ride" and distance < 10_000.0) and (
        is_close_to_office(activity)
    )
...
activites_to_mark_as_commute = list(
    filter(should_be_marked_as_commute, activities_non_processed)
)
